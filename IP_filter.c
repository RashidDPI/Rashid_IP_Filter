...
/* Simple forward. 8< */
static void
l2fwd_IP_forward(struct rte_mbuf *m, unsigned portid)
{
	unsigned dst_port;
	int sent;
    ...
    /* if (eh->ether_type == rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4))
		...
		{
		//printf("This is  data len: %d \n",len);
		//printf("This is packet lenth: %d \n",m->pkt_len);
		//printf("This is  IPV4 packet \n");
		//printf("This is IPV4 version header: %d \n",ip->version_ihl>>4);
		sent = rte_eth_tx_buffer(dst_port, 0, buffer, m);
		port_statistics[dst_port].tx += sent;
		}
